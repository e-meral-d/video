...
def load(name: str, device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu", design_details = None, jit: bool = False, download_root: str = None):
    ...
    if name in _MODELS:
        ...
        model_path = _download(_MODELS[name], download_root or os.path.expanduser("data"))
...
def get_similarity_map(sm, shape):
    ...
    if len(sm.shape) == 3:  # (B, N_patches, N_classes)
        b, n_patches, n_classes = sm.shape
        # Try to find a reasonable grid size
        grid_size = int(n_patches ** 0.5)
        if grid_size * grid_size != n_patches:
            # If not perfect square, use the closest approximation
            grid_size = int(n_patches ** 0.5)
        ...
        if grid_size * grid_size >= n_patches:
            ...
            padded_sm = torch.zeros(b, grid_size * grid_size, n_classes, device=sm.device, dtype=sm.dtype)
            padded_sm[:, :n_patches, :] = sm
            sm = padded_sm
        
        sm = sm.reshape(b, grid_size, grid_size, n_classes).permute(0, 3, 1, 2)
    else:
...
def compute_similarity(image_features, text_features, t=2):
    # image_features: (B, N_patches, D)
    # text_features: (N_classes, D) where N_classes=2 for normal/abnormal
    b, n_patches, d = image_features.shape
    ...
    # Compute similarity: (B, N_patches, N_classes)
    similarity = torch.matmul(image_norm, text_norm.t())  # (B, N_patches, N_classes)
    
    # For backward compatibility, also compute prob_1
    prob_1 = similarity[:, :1, :]  # (B, 1, N_classes)
