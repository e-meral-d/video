...
class FocalLoss(nn.Module):
    ...
    def forward(self, logit, target):
        ...
        # Handle target dimensions safely - only squeeze if target has more than 1 dimension
        if target.dim() > 1:
        ...
        idx = target.cpu().long().unsqueeze(1)  # Make idx 2D: (N, 1)
        ...
        # Ensure pt is positive and stable for log operation
